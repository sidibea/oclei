...
function stm_admin_changelog_scripts($hook)
{
	if ($hook == 'index.php') {
		...
		$assets = get_template_directory_uri() . '/includes/admin/announcement/assets/';
		wp_enqueue_style('milligram', $assets . 'custom.css', null, $theme_info['v'], 'all');

		wp_enqueue_script('vue.js', $assets . 'vue.min.js', null, $theme_info['v'], true);
		wp_enqueue_script('vue-resource.js', $assets . 'vue-resource.js', array('vue.js'), $theme_info['v'], true);
		wp_enqueue_script('pearl-vue.js', $assets . 'vue.js', array('vue.js'), $theme_info['v'], true);
	}
}