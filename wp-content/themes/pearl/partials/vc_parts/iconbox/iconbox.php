...
if (!empty($box_link) && $box_link === 'enable' && !empty($box_link_url)) {
	...
	if (!empty($link['url'])) {
		...
		$before_tpl .= "<a class='stm_iconbox__link' href='$link_url' title='$link_title' rel='$link_rel'>";
...
?>
    ...
    <div class="<?php echo esc_attr(implode(' ', $classes)); ?> clearfix">
        ...
        <div class="stm_iconbox__text">
			<?php if ($title) { ?>
                <h5 <?php echo esc_attr(implode(' ', $title_styles)); ?> class="<?php echo esc_attr($h_class) ?>"><span class="mtc_b"><?php echo sanitize_text_field($title); ?></span></h5>
			<?php } ?>
