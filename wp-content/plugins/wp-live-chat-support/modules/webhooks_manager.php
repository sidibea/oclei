...
function wplc_webhooks_manager_menu($cap) {
    ...
    if(isset($wplc_settings['wplc_use_node_server']) && $wplc_settings['wplc_use_node_server'] == 1){
		...
		if (current_user_can("manage_options")) {
			add_submenu_page('wplivechat-menu', __('Webhooks', 'wplivechat'), __('Webhooks', 'edit_posts'), $cap[0], 'wplivechat-menu-webhooks-page', 'wplc_webhooks_page');
		}
    }
}
...
function wplc_webhooks_table() {
    ...
    if($webhooks !== false){
        ...
        foreach ($webhooks as $key => $webhook) {
                ...
                $content .=     "<td>";
                $content .=         "<a href='?page=wplivechat-menu-webhooks-page&wplc_action=edit_webhook&id=".$webhook->id."' title='" . __("Edit", "wplivechat") . "' class='button button-secondary'>" . __("Edit", "wplivechat") . "</a> ";
                $content .=         "<a href='?page=wplivechat-menu-webhooks-page&wplc_action=delete_webhook&id=".$webhook->id."' title='" . __("Delete", "wplivechat") . "' class='button button-secondary'>" . __("Delete", "wplivechat") . "</a>";
                $content .=     "</td>";
                $content .= "</tr>";
        }
...
function wplc_webhooks_confirm_delete_prompt() {
  ...
  $content .=     "<tr>";
  $content .=         "<td>";
  $content .=             "<a href='?page=wplivechat-menu-webhooks-page&wplc_action=delete_webhook_confirmed&id=".$id."&wplc_webhook_nonce=" . $wplc_webhook_nonce ."' title='" . __("Confirm", "wplivechat") . "' class='button button-primary'>" . __("Confirm", "wplivechat") . "</a> ";
  $content .=             "<a href='?page=wplivechat-menu-webhooks-page' title='" . __("Cancel", "wplivechat") . "' class='button button-secondary'>" . __("Cancel", "wplivechat") . "</a> ";
  $content .=         "</td>";
  $content .=      "</tr>";
  $content .= "</table>";
  return $content;
