...
define( 'BOOKED_WC_VERSION', '1.4.9' ); // Updated in 2.0.10
define( 'BOOKED_FEA_VERSION', '1.1.15' ); // ---
define( 'BOOKED_CF_VERSION', '1.1.5' ); // Updated in 2.0.7
	...
	class booked_plugin {
				...
				<option value=""><?php esc_html_e('Default','booked'); ?></option><?php
					...
					if(!empty($allowed_users)) :
						foreach($allowed_users as $u) :
							...
							<option value="<?php echo $email; ?>"><?php echo $email; ?></option><?php
						endforeach;
					endif;
		...
		**********************/
		...
		public function front_end_scripts() {
			...
			$profile_page = ( $profile_page ? esc_url( get_permalink( $profile_page ) ) : ( $booked_detect_profile_page ? esc_url( get_permalink( $booked_detect_profile_page->ID ) ) : false ) );
